...
/*
  Create a constructor function called CarFactory that will build car objects with make and model 
  properties.
...
/*
  ...
  Hint: you'll need to add a move property,
    with a starting value of zero, and write a moveCar function which will increment the 
    move property 
    by 10.
...
Movie.prototype.changeRating = function(number){
  newRating = number;
  var updatedRating = Movie.rating/newRating;
...
//savedPosts will be an array of objects. These objects will each have 3 properties: id (a number), title 
//(a string), and rating (a number between 1 and 5). These objects are the posts that the user 
...
//that will take in one number parameter representing the post id. Use this id to find and remove the
// matching object in the savedPosts array.

// Code here

User.prototype.removeSavedPost = function(number){
  let index = this.savedPosts.findIndex(post => post.id === number);
  this.savedPosts.splice(index, 1)
  return this.savedPosts.slice;
}
